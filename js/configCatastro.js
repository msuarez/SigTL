/**
	...
	Armado y definici�n de mapa y capas
**/
/*
...
function init() {
			...
			//opciones para la capa Circunscripci�n
			var opCircuns =	{layers: 'Circunscripcion',		attribution: 'Circunscripci�n',		transparent: true, 	isBaseLayer:false, reproject:true};
...
*/			
			...
			var opOlMap = {maxResolution: 'auto', units: 'm', projection: projOlMap, displayProjection: projOlMap, maxExtent: misBounds};
...
//			olMap.addLayer(cATens);
			...
			//Capa satelite de google
 			var satelite = new OpenLayers.Layer.Google	("Ggl-Sat�lite", 
			...
			var fisico = new OpenLayers.Layer.Google("Ggl-F�sico",
...
//------------------- prueba con capa de marcadores
			...
			var size = new OpenLayers.Size(22,22);														//defino el tama�o que tendr� el icono del marcador
			var offset = new OpenLayers.Pixel(-(size.w/2), -size.h);									//defino el desplazamiento del icono respecto de la coordenada de ubicaci�n
			var icono = new OpenLayers.Icon("/test/imagenes/foco.png",size,offset);		//defino la imagen del marcador con ruta, tama�o y desplazamiento
...
//------------------- prueba con capas vectoriales
			...
			var feature_point_2 = new OpenLayers.Feature.Vector(
										coordPoint,
												...
												'description': 'Rotonda de rutas nacionales 5 y 33, destacamento polic�a caminera.'
												}
									);
			...
			var feature_polygon = new OpenLayers.Feature.Vector(//Parque municipal
																	...
																	'description': 'Paseo p�blico con lago artificial, anfiteatro, museo hist�rico, etc.'
																	}
																);
			...
			function selected_feature(event){
				...
				//mostrar la caracter�stica seleccionada (pasada desde el evento del objeto)
				var display_text = 'Selecci�n: '+ '<br><strong>' + event.feature.attributes.location + '</strong>'	+ ': ' + event.feature.attributes.description + '<hr/>';
				document.getElementById('footer').innerHTML =display_text;
				//mostrar todas las caracter�sticas seleccionadas
				document.getElementById('footer').innerHTML += 'Caracter�sticas Seleccionadas: <br>';
			...
			function unselected_feature(event){
				...
				document.getElementById('footer').innerHTML += 'Seleccionar Caracter�sticas: <br>';
				//recorrer el arreglo de caracteristicas seleccionadas
				for(var i=0; i<cVectores.selectedFeatures.length; i++){
					document.getElementById('footer').innerHTML += cVectores.selectedFeatures[i].attributes.location + ' | ';
				}
